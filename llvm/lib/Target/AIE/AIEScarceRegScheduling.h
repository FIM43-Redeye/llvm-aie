...
class BurstMostUrgentStrategy : public PostPipelinerStrategy {
  ...
  // Current burst index (which scarce range we're working on).
  size_t CurrentBurst = 0;

  // Track which SUnits are part of scarce ranges.
  SmallVector<bool, 64> IsScarceRangeMember;

  // Precomputed non-scarce predecessors for each burst.
  // BurstPredecessors[i] contains the set of non-scarce predecessor indices
  // for burst i.
  std::vector<std::unordered_set<int>> BurstPredecessors;

  // Count of unscheduled non-scarce predecessors for the current burst.
  int UnscheduledPredCount = 0;
...
private:
  // Precompute non-scarce predecessors for all bursts.
  void precomputeBurstPredecessors();

  // Check if SUnit is a non-scarce predecessor of the current burst.
  bool isCurrentBurstPredecessor(int SUIdx) const;

  // Advance to the next burst and update UnscheduledPredCount.
  void advanceBurst();
};
