...
class RegLiveRangeTracker; // This is in llvm namespace, not AIE
...
private:
  ...
  struct AllocState {
    ...
    /// Pre-computed metrics for all VRegs (reused across scoring attempts).
...
private:
  ...
  /// \param VReg The virtual register to compute metrics for.
  /// \param Masks The lane masks for this VReg.
  ...
  /// This properly handles aliasing - if a sub-register's super-register
  /// is available, the sub-register is also considered available.
  /// Additionally, it can derive registers by checking if all their
  /// subregisters are available.
  static std::vector<Register>
  getCandidatePhysRegs(unsigned VReg, const DenseSet<MCRegister> &AvailableRegs,
                       const MachineRegisterInfo &MRI,
                       const TargetRegisterInfo &TRI);
