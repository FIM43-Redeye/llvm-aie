...
void AIEPostRegAlloc::AllocState::init(
  ...
  for (const auto &[VReg, _] : LiveLanesByVReg) {
    UsedRCIds.insert(MRI.getRegClass(VReg)->getID());
  }
  ...
  // Pre-compute metrics for all VRegs once.
  this->AllMetrics.clear();
  for (const auto &[VReg, Masks] : LiveLanesByVReg) {
    AllMetrics[VReg] = AIEPostRegAlloc::computeMetrics(
        VReg, Masks, VRegInterferenceGraph, LiveLanesByVReg,
        RCInterferenceGraph, AvailableRegs, MRI, *InTRI);
  }
}

// Check if VReg can be placed in PhysReg without conflicts.
bool AIEPostRegAlloc::AllocState::canPlace(
    unsigned VReg, Register PhysReg, const AIE::LaneMaskVector &VRegMasks,
...
// Compute metrics for a virtual register.
AIEPostRegAlloc::VRegMetrics AIEPostRegAlloc::computeMetrics(
    unsigned VReg, const AIE::LaneMaskVector &Masks,
  ...
  // Count available registers in this register class.
  std::vector<Register> Candidates =
      getCandidatePhysRegs(VReg, AvailableRegs, MRI, TRI);
  Metrics.NumAvailableRegs = Candidates.size();

  return Metrics;
}

// Get allocatable physical registers for a virtual register.
std::vector<Register> AIEPostRegAlloc::getCandidatePhysRegs(
    unsigned VReg, const DenseSet<MCRegister> &AvailableRegs,
    const MachineRegisterInfo &MRI, const TargetRegisterInfo &TRI) {

  std::vector<Register> Candidates;

  // Get the register class for this VReg.
  const TargetRegisterClass *RC = MRI.getRegClass(VReg);

  if (RC) {
    // Iterate through the register class and check availability.
    // The AvailableRegs set already includes:
    // 1. Base registers from live ranges
    // 2. Their sub-registers
    // 3. Derived super-registers (when all sub-registers are available)
    // This derivation is done once in RegTracker::analyze().
...
AIEPostRegAlloc::AllocResult AIEPostRegAlloc::tryAllocate(
    const DenseMap<unsigned, AIE::LaneMaskVector> &LiveLanesByVReg,
    const DenseSet<MCRegister> &AvailableRegs, const TargetRegisterInfo &TRI,
  ...
  // Build sorted list of VRegs by difficulty.
  struct VRegInfo {
    unsigned VReg;
    unsigned Score;
    const AIE::LaneMaskVector *Masks;
    const TargetRegisterClass *RC;
  };

  // Score and collect VRegs using pre-computed metrics from State.
  ...
  for (const auto &[VReg, Masks] : LiveLanesByVReg) {
    ...
    Info.Masks = &Masks;
    Info.RC = MRI.getRegClass(VReg);
    VRegs.push_back(Info);
  }

  // Sort by descending score (hardest first).
  llvm::sort(VRegs, [](const VRegInfo &A, const VRegInfo &B) {
    return A.Score > B.Score;
  });

  // Try to allocate each VReg.
  for (const auto &VRI : VRegs) {
    unsigned VReg = VRI.VReg;
    const auto &VRegMasks = *VRI.Masks;
    const TargetRegisterClass *RC = VRI.RC;
    const auto &Metrics = State.AllMetrics[VReg];

    LLVM_DEBUG(dbgs() << "Allocating " << printReg(VReg, &TRI) << " class="
                      << TRI.getRegClassName(RC) << " (score=" << VRI.Score
    ...
    // Get candidate physical registers.
    std::vector<Register> Candidates =
        getCandidatePhysRegs(VReg, AvailableRegs, MRI, TRI);
...
bool AIEPostRegAlloc::allocate(
  ...
  for (const auto &Strategy : Strategies) {
    ...
    AllocResult Result = tryAllocate(LiveLanesByVReg, AvailableRegs, TRI, MRI,
