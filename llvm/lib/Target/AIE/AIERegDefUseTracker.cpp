...
void RegLiveRangeTracker::pruneByFullCoverage() {
  ...
  while (Changed) {
    ...
    // registers
    auto HasUncoveredAlias = [&](DenseSet<MCRegister> &LRRegs) {
...
void RegLiveRangeTracker::mergeAliasingLiveRanges(
  ...
  SmallVector<std::pair<MCRegister, unsigned>, 8> AliasingLiveRegs;
  for (auto &Entry : LiveRegs) {
    const MCRegister LiveReg = Entry.first;
    if (TRI->regsOverlap(DefReg, LiveReg)) {
      AliasingLiveRegs.push_back({LiveReg, Entry.second});
    }
  }
  ...
  for (auto &[LiveReg, LRIdx] : AliasingLiveRegs) {
    ToMerge.insert(LRIdx);
  }
...
void RegLiveRangeTracker::analyze(MachineBasicBlock &MBB,
  ...
  // Map from register to its current live range index
  DenseMap<MCRegister, unsigned> LiveRegs;
  ...
  for (MachineInstr *MI : llvm::reverse(SemanticOrder)) {
    ...
    // Process operands: defs first, then uses
    // In backward pass: defs kill liveness, uses start liveness

    // First process defs - they kill liveness
    for (MachineOperand &MO : MI->operands()) {
      ...
      // Step 1: Always create a def-only live range for this def
      const unsigned DefLRIdx = LiveRanges.size();
      LiveRanges.emplace_back();
      LiveRanges[DefLRIdx].addDef(&MO, 0); // SubRegIdx 0 relative to itself
      LiveRanges[DefLRIdx].BaseReg = Reg;
      OperandToLiveRange[&MO] = DefLRIdx;

      // Step 2: Merge with any aliasing live ranges
      mergeAliasingLiveRanges(DefLRIdx, Reg, LiveRegs, OperandToLiveRange);
    }

    // Then process uses - they start liveness
    for (MachineOperand &MO : MI->operands()) {
      if (!MO.isReg() || !MO.getReg().isPhysical() || MO.isDef() ||
          MO.isImplicit())
        continue;
  ...
  DEBUG_WITH_TYPE(DEBUG_TYPE,
                  dbgs() << "First-stage filtering: " << LiveRanges.size()
                         << " candidate ranges\n");
  ...
  for (const RegLiveRange &LR : LiveRanges) {
    ...
    if (LR.getNumDefs() == 0) {
      DEBUG_WITH_TYPE(DEBUG_TYPE, dbgs() << "  Filtered: no defs\n");
      continue;
    }
    ...
    if (!startsWithDefInBlock(LR)) {
      DEBUG_WITH_TYPE(DEBUG_TYPE,
                      dbgs() << "  Filtered: doesn't start with def\n");
      continue;
    }

    // Reject tied operands
    if (hasTiedOperands(LR)) {
      DEBUG_WITH_TYPE(DEBUG_TYPE, dbgs() << "  Filtered: has tied operands\n");
      continue;
    }
    ...
    DEBUG_WITH_TYPE(DEBUG_TYPE, dbgs() << "  Kept range with "
                                       << LR.getNumDefs() << " defs, "
                                       << LR.getNumUses() << " uses\n");
    SafeRanges.push_back(LR);
  }

  DEBUG_WITH_TYPE(DEBUG_TYPE, dbgs() << "After first-stage: "
                                     << SafeRanges.size() << " safe ranges\n");
  ...
  // Compute register classes based on instruction constraints
  // Filter out live ranges that have no valid register class
  ...
  for (RegLiveRange &LR : LiveRanges) {
    ...
    if (LR.RegisterClass) {
      ...
      DEBUG_WITH_TYPE(
          DEBUG_TYPE,
          dbgs()
              << "  Filtered: no valid register class for all occurrences\n");
    }
  }
  ...
  for (const RegLiveRange &LR : LiveRanges) {
    ...
    MCSubRegIterator SubRegIt(LR.BaseReg, TRI);
    if (SubRegIt.isValid()) {
      for (MCRegister SubReg : TRI->subregs_inclusive(LR.BaseReg)) {
        AvailablePhysRegs.insert(SubReg);
      }
    } else {
...
void RegLiveRangeTracker::virtualizeFilteredPhysRegs() {
  ...
  // Mark as virtualized even if no live ranges were found
...
void RegLiveRangeTracker::dump() const {
  dbgs() << "Register Live Range Information:\n";
  dbgs() << "================================\n";
  ...
  for (size_t SortedIdx = 0; SortedIdx < SortedIndices.size(); ++SortedIdx) {
    ...
    dbgs() << "Live Range [" << SortedIdx << "] for " << PrimaryReg << ":\n";
